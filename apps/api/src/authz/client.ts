...
const endpoint = process.env.SPICEDB_GRPC_ADDRESS || 'localhost:50051';
...
function parseRef(input: string, fallbackType: string): { type: string; id: string } {
...
export async function checkPermission(subject: string, permission: string, resource: string): Promise<boolean> {
  try {
    ...
    const req = v1.CheckPermissionRequest.create({
      resource: v1.ObjectReference.create({ objectType: resRef.type, objectId: resRef.id }),
      subject: v1.SubjectReference.create({
        object: v1.ObjectReference.create({ objectType: subj.type, objectId: subj.id })
      }),
      permission
    });
...
export async function writeRelationship(namespace: string, objectId: string, relation: string, subjectId: string): Promise<void> {
  try {
    const md = makeAuthMetadata();
    const update = v1.RelationshipUpdate.create({
      operation: v1.RelationshipUpdate_Operation.CREATE,
      relationship: v1.Relationship.create({
        resource: v1.ObjectReference.create({ objectType: namespace, objectId }),
        relation,
        subject: v1.SubjectReference.create({
