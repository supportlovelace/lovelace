...
dashboardRoute.get('/my-games', async (c) => {
  ...
  try {
    ...
    for (const game of allGames) {
      ...
      try {
        const checkResponse = await spicedbClient.promises.checkPermission(checkRequest, md);
        if (checkResponse.permissionship === v1.CheckPermissionResponse_Permissionship.HAS_PERMISSION) {
    ...
    console.error('❌ Erreur lors de la récupération des jeux utilisateur:', error);
