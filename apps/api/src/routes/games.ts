...
import { checkPermission, writeRelationship, spicedbClient } from '../authz/client';
...
export const gamesRoute = new Hono<{ Variables: Variables }>()
  // GET /games - Lister tous les jeux (pour le moment, sans filtrage)
  .get('/', async (c) => {
    try {
  ...
  .post('/', async (c) => {
    try {
      ...
      // Écrire les deux relations SpiceDB pour l'héritage
      
      // 1. game:[ID] admin user:[CreatorID]
      await writeRelationship('game', game.id, 'admin', userId);
      
      // 2. game:[ID] studio studio:[StudioID]
      const md = makeAuthMetadata();
      const update = v1.RelationshipUpdate.create({
        operation: v1.RelationshipUpdate_Operation.CREATE,
        relationship: v1.Relationship.create({
          resource: v1.ObjectReference.create({ objectType: 'game', objectId: game.id }),
          relation: 'studio',
          subject: v1.SubjectReference.create({
            object: v1.ObjectReference.create({ objectType: 'studio', objectId: studioId })
          })
        })
      });
